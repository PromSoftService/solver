...
p = Path('tsgpu-worker.ps1')
s = p.read_text(encoding='utf-8')

old1 = """    $ipActions = Split-Actions (Invoke-Bridge $socket 'solver.node.actionsAfter' '/api/actions-after' 'POST' ([ordered]@{ append = @() }) 10000 $transcript)
...
"""
new1 = """    $ipActions = Split-Actions (Invoke-Bridge $socket 'solver.node.actionsAfter' '/api/actions-after' 'POST' ([ordered]@{ append = @() }) 10000 $transcript)
...
old2 = """    if ($DecisionNode -eq 'UTG_CBET') {
...
"""
new2 = """    if ($DecisionNode -eq 'UTG_CBET') {
        $checkSlot = Get-ActionSlot $validActions 'Check'

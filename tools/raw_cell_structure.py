#!/usr/bin/env python3
"""EXP-004: describe saved solver cells. NEVER optimize/select a human policy.
Separate entry point to avoid the old tools/universal_strategy/** CI trigger.
"""
...
def checkpoint(name: str, data: dict) -> None:
    core.dump(DEST / name, data)


def quantile(values: np.ndarray, w: np.ndarray, q: float) -> float:
    order = np.argsort(values, kind='stable')
    i = int(np.searchsorted(np.cumsum(w[order]), q * w.sum(), side='left'))
    return float(values[order[min(i, len(order)-1)]])


def profile(d: dict, mask: np.ndarray, W: float, class_w: float) -> dict:
    positive = mask & (d['w'] > 0)
    w = d['w'][positive]
    mass = float(w.sum())
    ...
    if not len(w):
        out.update(status='no_rows' if not mask.any() else 'zero_reach', frequencies=None,
                   action_ev_bb=None, mixed_ev_bb=None, mean_best_ev_bb=None,
                   forced_action_local_regret_bb=None, solver_local_regret_bb=None,
                   extra_loss_vs_mixed_bb=None, action_loss_p95_bb=None,
                   action_loss_mass_gt_05=None, action_loss_mass_gt_10=None,
                   frequency_p10=None, frequency_p90=None)
        return out
    ...
    out.update(status='observed', frequencies=vec(mean(freq)), action_ev_bb=vec(mean(ev)),
               mixed_ev_bb=float(mean(d['mixed'][positive])),
               mean_best_ev_bb=float(mean(ev.max(axis=1))),
               ...
               frequency_p10=vec([quantile(freq[:,i],w,.10) for i in range(len(d['actions']))]),
               frequency_p90=vec([quantile(freq[:,i],w,.90) for i in range(len(d['actions']))]))
    return out


def partition() -> list[dict]:
    if blob_sha(Path(core.__file__)) != EXPECTED_CORE:
        raise ValueError('core.py changed: review definitions before running EXP-004')
    boards = [' '.join(r+s for r,s in zip(rs,'shd'))
              for rs in itertools.combinations('AKQJT98765432',3)]
    def predicates(b):
        hi,mid,lo = core.board_features(b)
        return (hi==14 and lo>=10, hi==14 and mid>=12 and lo<10,
                hi==14 and mid in (11,10) and 5<=lo<=9,
                hi==14 and mid in (11,10) and lo<=4,
                hi==14 and 7<=mid<=9, hi==14 and mid<=6,
                hi<14 and lo>=10, hi<14 and mid>=10 and lo<=9,
                hi in (13,12) and mid<=9 and mid-lo!=1,
                hi in (13,12) and mid<=9 and mid-lo==1,
                8<=hi<=11 and mid<=9 and mid-lo!=1,
                8<=hi<=11 and mid<=9 and mid-lo==1, hi<=7)
    for b in boards:
        flags = predicates(b)
        if sum(flags)!=1 or core.B13[flags.index(True)]!=core.board_class(b,'thirteen'):
            raise ValueError(f'Partition disagreement: {b}')
    result=[]
    ...
    if sum(x['board_count'] for x in result)!=286:
        raise ValueError('Incomplete board partition')
    return result


def analyze(study: str) -> dict:
    ...
    result=dict(study=study,title=TITLES[study],file=d['file'],sha256=d['sha256'],
                ...
                actions=list(d['actions']),row_count=len(d['w']),
                board_count=len(set(d['board'])),classes={},cells={})
    ...
    for c in core.B13:
        ...
        cell_w=sum(v['reach_weight'] for v in result['cells'][c].values())
        if not np.isclose(cell_w,cw,rtol=1e-10,atol=1e-8):
            raise ValueError('Cell reach totals disagree')
        for a in d['actions']:
            weighted=sum(v['reach_weight']*v['frequencies'][a]
                         for v in result['cells'][c].values() if v['frequencies'] is not None)
    ...
    if result['board_count']!=286:
        raise ValueError('Wrong dataset coverage')
    checkpoint(study+'.json',result)
    print('DONE',study,'195 cells',result['overall']['frequencies'],flush=True)
    return result


def compare_columns(study: dict, left: str, right: str) -> dict:
    """Descriptive profile distance, NOT merge regret or a policy test."""
    pairs=[]
    coverage=[0.,0.]
    for h in core.HANDS:
        p,q=study['cells'][left][h],study['cells'][right][h]
        if p['frequencies'] is None or q['frequencies'] is None:
            continue
        a,b=p['share_of_class'],q['share_of_class']
        coverage[0]+=a;coverage[1]+=b
        diffs={act:100*abs(p['frequencies'][act]-q['frequencies'][act]) for act in study['actions']}
        lossdiff=max(abs(p['forced_action_local_regret_bb'][act]-q['forced_action_local_regret_bb'][act])
                     for act in study['actions'])
        pairs.append(dict(hand=h,weight=(a+b)/2,tv_pp=sum(diffs.values())/2,
                          max_action_difference_pp=max(diffs.values()),
                          max_loss_difference_bb=lossdiff,material=(a>=.02 and b>=.02),
                          left_share=a,right_share=b))
    norm=sum(x['weight'] for x in pairs)
    material=[x for x in pairs if x['material']]
    worst=max(material,key=lambda x:x['max_action_difference_pp']) if material else None
    return dict(left=left,right=right,common_hand_coverage=coverage,
                ...
                worst_material_hand=worst,rows=pairs)


def table(st: dict, field: str, action: str | None, percent=False) -> str:
    def row(label,vals):
        # Separator AFTER the ace-high block, same location in every matrix.
        vals=vals[:6]+['│']+vals[6:]
        return '| '+label+' | '+' | '.join(vals)+' |'
    out=[row('Рука',list(core.B13)),row('---',[':---:']*len(core.B13))]
    for i,h in enumerate(core.HANDS):
        if i in (7,12):
            out.append(row('**────────**',['─']*len(core.B13)))
        values=[]
        for c in core.B13:
            cell=st['cells'][c][h]
            if cell['status']!='observed':
                values.append('—' if cell['status']=='no_rows' else '∅')
                continue
            v=cell[field]
            ...
            if field=='frequencies' and action is None:
                ...
                values.append(f'{100*v:.1f}' if percent else f'{v:.4f}')
...
def render(result: dict) -> None:
    head=['# EXP-004 — исходные профили solver: 7 спотов × 13 флопов × 15 рук','',
          ...
          'Вес = reach_probability, нормированный отдельно в каждом study. Нулевая частота действия не означает отсутствующую руку. '
          '— = нет строк в dataset; ∅ = строки есть, но суммарный reach равен нулю. '
          'Не суммировать веса разных спотов. Проценты округлены только для отображения; JSON/CSV содержат полную точность.','',
          'Для BET-спотов показан BET%, CHECK=100−BET. Для защиты каждая ячейка — **FOLD/CALL/RAISE %**. '
    ...
    head += ['', '## Определения рук и ограничения', '',
             ...
             'Combo draw = прямое стрит-дро + BDFD; OESD включает double gutshot. Затем остаточные BDFD, 2OC и Air. '
             'Таким образом строки непересекаются, но утверждённый порядок отображения НЕ является точным ранжированием чистых аутов/силы. '
             '2OC+draw может содержать сильное дро, хотя строка ниже BDFD. Это ограничение сохранено явно, без молчаливого переопределения.', '',
             'loss_if_action = среднее по reach(best pure EV − action EV). Это не best(mean EV) − mean(action EV). '
             'Дополнительная потеря против экспортированной mixed-стратегии также сохранена отдельно. '
             'Проверки BR/strategy-lock нет. Equity не используется.', '']
    compact=list(head)
    full=list(head)
    flat=[]
    for key,st in result['studies'].items():
        lines=['## '+TITLES[key],'', '### Solver frequencies','']
        lines += [table(st,'frequencies','B' if st['actions']==['X','B'] else None,True),'']
        lines += ['### Reach класса и частоты всего диапазона','',
        ...
        for a in st['actions']:
            full += ['### Forced '+core.ACTION_NAMES[a].upper()+': local regret, bb','',
                     table(st,'forced_action_local_regret_bb',a),'']
        for c,hands in st['cells'].items():
            for h,p in hands.items():
                ...
                for field in ('frequencies','action_ev_bb','forced_action_local_regret_bb','extra_loss_vs_mixed_bb',
                    ...
                    for a in st['actions']:
                        item[field+'_'+a]=p[field][a] if p[field] is not None else None
                item['mixed_ev_bb']=p['mixed_ev_bb']
                flat.append(item)
    comparisons=['## Сходство соседних столбцов: только диагностика','',
                 'Сначала сравниваются одинаковые строки рук. Средняя TV — half-L1 разница частот, '
                 'взвешенная средним составом двух столбцов на общих строках. '
                 'Значимая строка для максимума: >=2% reach В КАЖДОМ из двух классов. '
                 'Это диагностический порог, не покерный закон. Общие строки могут покрывать не весь reach — покрытие раскрыто. '
    ...
    for key,diag in result['neighbor_diagnostics'].items():
        for p in diag:
            worst=p['worst_material_hand']
            mean=p['mean_hand_profile_tv_pp']
...
def main() -> None:
    ...
    result=dict(experiment='EXP-004',mode='raw_solver_profiles_no_human_policy',
                ...
                input_commit=os.environ.get('GITHUB_SHA','local'),
                classifier_git_blob=EXPECTED_CORE,hand_order=list(core.HANDS),
                flops=partition(),studies={},neighbor_diagnostics={})
    for study in core.FILES:
        result['studies'][study]=analyze(study)
    for key,st in result['studies'].items():
        result['neighbor_diagnostics'][key]=[compare_columns(st,l,r) for l,r in zip(core.B13,core.B13[1:])]
    core.dump(BASE/'EXP-004.json',result)
    render(result)
    ...
    if len(result['studies'])!=7:
        raise ValueError('Expected seven independent studies')

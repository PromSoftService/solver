...
const branchInfo = [
  { id: "01_BB_FIRST", sheet: "01 BB first", title: "BB: чек или донк 50%", actor: "BB" },
  { id: "02_UTG_AFTER_CHECK", sheet: "02 UTG cbet", title: "UTG после чека BB", actor: "UTG" },
  { id: "03_BB_AFTER_CBET", sheet: "03 BB vs cbet", title: "BB против c-bet UTG 50%", actor: "BB" },
  { id: "04_UTG_AFTER_CHECK_RAISE", sheet: "04 UTG vs XR", title: "UTG против check-raise BB", actor: "UTG" },
  { id: "05_UTG_AFTER_DONK", sheet: "05 UTG vs donk", title: "UTG против донка BB 50%", actor: "UTG" },
  { id: "06_BB_AFTER_DONK_RAISE", sheet: "06 BB vs donk R", title: "BB против рейза UTG после донка", actor: "BB" },
];
...
function addSummary(workbook, model, loaded) {
  ...
  sheet.getRange("A2").values = [["STU002 — стратегия флопа UTG vs BB"]];
  ...
  sheet.getRange("A6:J11").values = rows;
  sheet.getRange("A6:J11").format = {
    fill: colors.background,
    font: { name: "Aptos", size: 10, color: colors.text },
    borders: { preset: "all", style: "thin", color: colors.border },
    wrapText: true,
  };
  sheet.getRange("C6:C11").format.numberFormat = "0.0%";
  sheet.getRange("F6:H11").format.numberFormat = "0.000000";
  sheet.getRange("I6:J11").format.numberFormat = "0";

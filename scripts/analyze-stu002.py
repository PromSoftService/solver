...
def write_outputs(branch: str, meta: dict, result: dict) -> None:
    out = OUT_ROOT / branch
    out.mkdir(parents=True, exist_ok=True)
    write_csv(out / "strategy.csv", result["matrix"], ["hand_category"] + B13)
    detail_fields = ["flop_class", "hand_category", "boards_present", "active_combos",
                     "reach_sum"] + [f"mean_{a}_frequency" for a in meta["actions"]]
    detail_fields += ["policy", "mean_frequency_distance",
                      "mean_local_loss_bb", "max_combo_loss_bb"]
    write_csv(out / "cell-details.csv", result["details"], detail_fields)
    top_loss = sorted(result["details"], key=lambda row: row["mean_local_loss_bb"], reverse=True)[:10]
    write_csv(out / "top-loss.csv", top_loss, detail_fields)
    with (out / "summary.json").open("w", encoding="utf-8") as f:
        json.dump(result["summary"], f, indent=2, ensure_ascii=False)


def write_methodology() -> None:
    text = f"""# STU002 simplified flop strategy
...
If the largest mean action frequency is greater than {PURE_THRESHOLD:.0%}, the
...
"""
    (OUT_ROOT / "METHODOLOGY.md").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    for branch, meta in BRANCHES.items():
        ...
        result = analyze_branch(branch, meta, rows)
        write_outputs(branch, meta, result)
    ...
    with (OUT_ROOT / "validation.json").open("w", encoding="utf-8") as f:
        json.dump(audit, f, indent=2, ensure_ascii=False)
    with (OUT_ROOT / "summary.json").open("w", encoding="utf-8") as f:
        json.dump(summaries, f, indent=2, ensure_ascii=False)
    write_methodology()
    print(json.dumps({"status": "PASS", "b13": audit["b13_counts"],
                      "branches": summaries}, ensure_ascii=False))

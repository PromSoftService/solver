...
def branch_meta(branch: dict) -> dict:
    shape = branch["action_shape"]
    if shape not in ACTION_SHAPES:
        raise RuntimeError(f"Unsupported action shape: {shape}")
    return {
        **ACTION_SHAPES[shape],
        "shape": shape,
        "actor": branch["acting_player"],
        "branch": branch["id"],
        "decision_node": branch["decision_node"],
    }
...
def read_branch(
    branch: str, meta: dict, run: Path, run_label: str | None = None
    ...
    validation["run"] = run_label or run.name

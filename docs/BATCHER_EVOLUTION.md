...
10. **Human simplification research.** Candidate flop partitions and subgroup selectors were tested manually from existing combo data. STU002 now has documented approved examples for the four-class BB response to c-bet and the three-class UTG responses to check-raise and donk; no production script generates or consumes them, and canonical solver workbooks remain unchanged.

...
10. **Human simplification research.** Candidate flop partitions and subgroup selectors were tested manually from existing combo data. STU002 has approved examples for the four-class BB response to c-bet, the three-class UTG responses to check-raise and donk, and the three-class BB response to the donk-raise. STU004 has an approved four-class UTG response to BTN stab. No production script generates or consumes these tables, and canonical solver workbooks remain unchanged.

...
`NOD002` is the UTG flop c-bet study requested for `CFG001`. The tree and ranges are unchanged: UTG has only `Check` and `Bet 33%` at the target node. With the x10 native money scale the runner validates `Bet 18`, then exports the UTG strategy **before** applying that bet.
...
The third command is the new UTG c-bet study.
...
Runs with an explicit decision ID include it in the name. For the UTG c-bet study:
...
Every run stores a `RUN_MANIFEST.json` with IDs, decision-node semantics, input filenames, SHA-256 hashes, money scale, expected action amounts, and board count. A copy of the manifest is stored next to the analysis dataset.
...
`NOD002 / UTG_CBET` uses the UTG schema:
